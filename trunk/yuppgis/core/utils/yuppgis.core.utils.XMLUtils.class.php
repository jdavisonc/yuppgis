...
/**
 * Clase que brinda utilidades sobre XMLs
 * 
...
class XMLUtils {
	...
	/**
	 * Funcion que compara dos XMLs, obtenida de http://www.jevon.org/wiki/Comparing_Two_SimpleXML_Documents
	 * @param SimpleXMLElement $xml1
	 * @param SimpleXMLElement $xml2
	 * @param unknown_type $text_strict
	 */
	public static function xml_is_equal(SimpleXMLElement $xml1, SimpleXMLElement $xml2, $text_strict = false) {
		...
		foreach ($search1 as $child_name => $children) {
			...
			foreach ($children as $child) {
				...
				foreach ($search2[$child_name] as $id => $second_child) {
					if (($r = self::xml_is_equal($child, $second_child)) === true) {
		...
		foreach ($ns1 as $ns) {			// don't need to cycle over ns2, since its identical to ns1
			...
			foreach ($search1 as $child_name => $children) {
				...
				foreach ($children as $child) {
					...
					$found_match = false;
					foreach ($search2[$child_name] as $id => $second_child) {
						if (self::xml_is_equal($child, $second_child) === true) {
							// found a match: delete second
							$found_match = true;
							unset($search2[$child_name][$id]);
						}
